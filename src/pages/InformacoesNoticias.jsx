...
function InformacoesNoticias() {
  ...
  const carregarNoticias = async (direcao = 'inicial') => {
    try {
      let baseQuery = collection(db, 'informacoes', 'noticias', 'itens');
      let q;
      ...
      console.error(err);
      setMensagem('Erro ao carregar notícias.');
    }
  };

  const excluirNoticia = async (id, midiaUrl) => {
    if (!window.confirm('Deseja excluir esta notícia e sua mídia?')) return;
    ...
    try {
      ...
      await deleteDoc(doc(db, 'informacoes/noticias/itens', id));
      setMensagem('Notícia excluída com sucesso!');
      carregarNoticias();
    } catch (err) {
      console.error(err);
      setMensagem('Erro ao excluir notícia ou mídia.');
    }
  };
  ...
  const salvarEdicao = async () => {
    ...
    try {
      const ref = doc(db, 'informacoes/noticias/itens', editando);
      ...
      <div className="container mt-4">
              ...
              <div className="card">
                ...
                <div className="card-body">
                  ...
                  <button
                    ...
                    onClick={() => excluirNoticia(e.id, e.midiaUrl)}
                  >
                    Excluir
                  </button>
                </div>
        ...
        <div className="d-flex justify-content-between mt-3">
          <button
            ...
            onClick={() => carregarNoticias('anterior')}
          >
            Anterior
          </button>
          <button
            className="btn btn-outline-primary"
            disabled={noticias.length < limite}
            onClick={() => carregarNoticias('proxima')}
          >
            Próxima
          </button>
        </div>
...
export default InformacoesNoticias;
